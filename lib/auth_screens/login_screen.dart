...
class _LoginScreenState extends State<LoginScreen>
  ...
  Future<void> _carregarNoticias() async {
    if (!_showNoticias) { if (mounted) setState(() => _loadingNoticias = false); return; }
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        list.sort((a, b) {
          final da = a['dtNoticia']?.toString() ?? a['dtImport']?.toString() ?? '';
          final db = b['dtNoticia']?.toString() ?? b['dtImport']?.toString() ?? '';
  ...
  void _goHome() {
    if (kIsWeb) {
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const WebBottomNavBarScreen()), (_) => false);
    } else if (defaultTargetPlatform == TargetPlatform.windows) {
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const WindowsBottomNavBarScreen()), (_) => false);
    } else {
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const BottomNavBarScreen()), (_) => false);
    }
  }
  ...
  Future<void> _login() async {
    ...
    if (resp.isSuccess && resp.body != null) {
      ...
      if (model.token == null || model.token!.isEmpty) {
        if (mounted) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Token nao recebido'))); }
        return;
      }
      ...
      final msg = resp.statusCode == 400 || resp.statusCode == 401
          ? 'Email ou senha invalidos' : resp.statusCode == -1 ? 'Sem conexao com o servidor' : 'Erro ${resp.statusCode}';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: [
        _LoginBanner(
          emailCtrl: _emailController, passCtrl: _passwordController, formKey: _formKey,
          obscure: _obscurePassword, onToggleObscure: () => setState(() => _obscurePassword = !_obscurePassword),
          loading: _loginInProgress, onLogin: _login,
          onForgot: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const EmailVarificationScreeen())),
        ),
        if (_showNoticias) Expanded(child: _NoticiasGrid(noticias: _noticias, loading: _loadingNoticias)),
        if (_showNoticias) const _EmpresaFooter(),
      ]),
    );
  }
}
...
class _LoginBanner extends StatelessWidget {
  ...
  final VoidCallback onToggleObscure, onLogin, onForgot;
  const _LoginBanner({required this.emailCtrl, required this.passCtrl, required this.formKey,
    required this.obscure, required this.loading, required this.onToggleObscure, required this.onLogin, required this.onForgot});
  ...
  Widget build(BuildContext context) {
    return Container(
      color: _green, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
      child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        Container(
          ...
          child: Image.asset('assets/images/Logo contabilidade_page-0001.jpg', height: 80, fit: BoxFit.contain),
        ),
        const SizedBox(width: 16),
        const Text('ABRACO\nCONTABILIDADE', textAlign: TextAlign.left,
            style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 1.5, height: 1.3)),
        const SizedBox(width: 32),
        Expanded(child: Form(key: formKey, child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          Expanded(child: Column(mainAxisSize: MainAxisSize.min, children: [
            _field(ctrl: emailCtrl, hint: 'Email', icon: Icons.email_outlined,
                validator: (v) => (v == null || v.isEmpty) ? 'Informe o email' : null),
            const SizedBox(height: 8),
            _field(ctrl: passCtrl, hint: 'Senha', icon: Icons.lock_outline, obscure: obscure,
                suffix: IconButton(onPressed: onToggleObscure,
                    icon: Icon(obscure ? Icons.visibility_off : Icons.visibility, color: Colors.white60, size: 18)),
          ...
          Column(mainAxisSize: MainAxisSize.min, children: [
            SizedBox(height: 44, child: ElevatedButton(
              ...
              child: loading ? const SizedBox(width: 18, height: 18,
                  ...
                  : const Text('Acessar', style: TextStyle(fontWeight: FontWeight.bold)),
            )),
            TextButton(onPressed: onForgot,
                style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2), minimumSize: Size.zero),
                child: const Text('Esqueceu a senha?', style: TextStyle(color: Colors.white54, fontSize: 10))),
          ]),
        ]))),
      ]),
    );
  ...
  Widget _field({required TextEditingController ctrl, required String hint, required IconData icon,
    ...
    return TextFormField(controller: ctrl, obscureText: obscure,
      style: const TextStyle(color: Colors.white, fontSize: 13), validator: validator,
      decoration: InputDecoration(hintText: hint, hintStyle: const TextStyle(color: Colors.white38, fontSize: 13),
        prefixIcon: Icon(icon, color: Colors.white54, size: 18), suffixIcon: suffix, isDense: true,
        contentPadding: const EdgeInsets.symmetric(horizontal: 10, vertical: 12),
        filled: true, fillColor: Colors.white.withValues(alpha: 0.1),
        enabledBorder: OutlineInputBorder(borderSide: const BorderSide(color: _red, width: 1.5), borderRadius: BorderRadius.circular(8)),
        focusedBorder: OutlineInputBorder(borderSide: const BorderSide(color: _red, width: 2), borderRadius: BorderRadius.circular(8)),
        errorBorder: OutlineInputBorder(borderSide: const BorderSide(color: Colors.orange, width: 1.5), borderRadius: BorderRadius.circular(8)),
        focusedErrorBorder: OutlineInputBorder(borderSide: const BorderSide(color: Colors.orange, width: 2), borderRadius: BorderRadius.circular(8)),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(color: _green, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
          child: Row(children: [
            ...
            const Text('contabeis.com.br', style: TextStyle(color: Colors.white54, fontSize: 11)),
          ])),
        Container(height: 2, color: _red),
        Expanded(child: loading
            ? Center(child: CircularProgressIndicator(color: _green))
            : noticias.isEmpty
                ? const Center(child: Text('Nenhuma noticia disponivel.', style: TextStyle(color: Colors.grey)))
                : _buildGrid(context)),
      ]),
    );
  }
...
class _NoticiaCard extends StatelessWidget {
  ...
  static Widget _placeholder() {
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(colors: [Color(0xFF005826), Color(0xFF2E7D32)],
          begin: Alignment.topLeft, end: Alignment.bottomRight)),
      child: const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.newspaper, color: Colors.white38, size: 36),
        SizedBox(height: 4),
        Text('Contabeis', style: TextStyle(color: Colors.white30, fontSize: 10, fontWeight: FontWeight.bold)),
      ])),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: _abrirLink, borderRadius: BorderRadius.circular(8),
      child: Container(
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              Text(titulo, maxLines: 2, overflow: TextOverflow.ellipsis,
                  style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1B1B1B), height: 1.3)),
              ...
              Row(children: [
                if (data.isNotEmpty) Text(data, style: const TextStyle(fontSize: 10, color: Colors.grey)),
                const Spacer(),
                const Icon(Icons.open_in_new, size: 11, color: _red),
                const SizedBox(width: 3),
                const Text('Ler mais', style: TextStyle(fontSize: 10, color: _red, fontWeight: FontWeight.w700)),
              ]),
            ]),
          ),
        ]),
  ...
  String _formatDate(dynamic dt) {
    if (dt == null) return '';
    final s = dt.toString();
    if (s.length >= 10) { final p = s.substring(0, 10).split('-'); if (p.length == 3) return '${p[2]}/${p[1]}/${p[0]}'; }
    return '';
  }
}
...
class _EmpresaFooter extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(color: _red, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
      child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        Expanded(child: Wrap(spacing: 24, runSpacing: 4, crossAxisAlignment: WrapCrossAlignment.center, children: [
          ...
          _infoItem(Icons.location_on, 'Rua Marques do Parana, 250 - Estados Unidos, Uberaba - MG'),
          _infoItem(Icons.phone, '(34) 3321-6689'),
          _infoItem(Icons.access_time, 'Seg-Sex: 09:00 - 18:00'),
        ])),
        const SizedBox(width: 16),
        OutlinedButton.icon(
          onPressed: () => _abrirLink('https://github.com/wlclimaco85/task_manager_flutter/releases/tag/latest-windows'),
          icon: const Icon(Icons.download, size: 16, color: Colors.white70),
          label: const Text('Download Windows', style: TextStyle(fontSize: 11, color: Colors.white70)),
  ...
  Widget _infoItem(IconData icon, String text) => Row(mainAxisSize: MainAxisSize.min, children: [
    Icon(icon, size: 12, color: Colors.white60), const SizedBox(width: 4),
