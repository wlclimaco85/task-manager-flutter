...
class PedidoVendaFormDialog extends StatefulWidget {
  ...
  const PedidoVendaFormDialog({super.key, this.item, required this.onSaved});

  @override
  State<PedidoVendaFormDialog> createState() => _PedidoVendaFormDialogState();
}

class _PedidoVendaFormDialogState extends State<PedidoVendaFormDialog> {
  ...
  void initState() {
    ...
    if (widget.item != null) {
      ...
      _orcamentoId = item['orcamentoId'];
      _orcamentoNumero = item['orcamentoNumero'];
      _reservarEstoque = item['reservarEstoque'] ?? false;
  ...
  Future<void> _loadOrcamentosAprovados() async {
    try {
      ...
      url = '$url?status=APROVADO';
      final response = await NetworkCaller().getRequest(url);
      if (response.isSuccess && response.body != null) {
        final data = response.body!['data']?['dados'] ?? response.body!['data'] ?? [];
        if (data is List) {
          _orcamentosAprovados = data.map((e) => Map<String, dynamic>.from(e)).toList();
        }
      }
    } catch (_) {}
    if (mounted) setState(() => _loadingOrcamentos = false);
  }
  ...
  Future<void> _save() async {
    ...
    final success = widget.item != null
        ? await PedidoVendaService.update(widget.item!['id'], payload)
        : await PedidoVendaService.create(payload);
      ...
      child: ConstrainedBox(
        ...
        child: Form(
          ...
          child: Column(
            children: [
              AppBar(
                title: Text(widget.item != null ? 'Editar Pedido de Venda' : 'Novo Pedido de Venda'),
                    ...
                    children: [
                      ...
                      _row([
                        ...
                        CheckboxListTile(
                          ...
                          onChanged: (v) => setState(() => _reservarEstoque = v ?? false),
                          title: const Text('Reservar Estoque', style: TextStyle(fontSize: 13)),
  ...
  Widget _orcamentoDropdown() {
    return SizedBox(
      ...
      child: DropdownButtonFormField<int>(
        ...
        isDense: true,
        decoration: const InputDecoration(
          labelText: 'Vínculo com Orçamento (opcional)',
          border: OutlineInputBorder(),
        ),
        items: [
          const DropdownMenuItem(value: null, child: Text('Sem vínculo')),
          ..._orcamentosAprovados.map((o) {
            final id = o['id'] as int?;
            final label = '${o['numero'] ?? '#'} - ${o['clienteNome'] ?? ''}';
        ...
        onChanged: (v) {
          setState(() {
            _orcamentoId = v;
            if (v != null) {
              final orc = _orcamentosAprovados.firstWhere((o) => o['id'] == v);
              _orcamentoNumero = orc['numero']?.toString();
              _clienteId = orc['clienteId'];
              _clienteNome = orc['clienteNome'];
              if (orc['itens'] != null) {
                _itens.clear();
                for (final i in orc['itens']) {
                  _itens.add(_ItemRow(
                    produtoId: i['produtoId'],
                    produtoNome: i['produtoNome'],
                    descricaoCtrl: TextEditingController(text: i['descricao'] ?? ''),
