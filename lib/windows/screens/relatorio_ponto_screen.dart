                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                            ...
                            label: const Text('Relatório de Absenteísmo', style: TextStyle(color: Colors.white, fontSize: 16)),
                            onPressed: _loading ? null : () async {
                              setState(() => _loading = true);
                              final dados = await PontoService.relatorioAbsenteismo(_mesSelecionado, _anoSelecionado);
                              if (!mounted) return;
                              setState(() => _loading = false);
                              showDialog(
                                context: context,
                                builder: (_) => AlertDialog(
                                  title: const Text('Absenteísmo'),
                                  content: SizedBox(
                                    width: double.maxFinite,
                                    child: dados.isEmpty ? const Text('Sem registros de faltas/atrasos neste período.') : ListView.builder(
                                      shrinkWrap: true,
                                      itemCount: dados.length,
                                      itemBuilder: (c, i) {
                                        final d = dados[i];
                                        return ListTile(
                                          title: Text(d['funcionario'] ?? ''),
                                          subtitle: Text("Faltas: ${d['faltas']} | Atrasos: ${d['atrasos']}"),
                                        );
                                      },
                                    ),
                                  ),
