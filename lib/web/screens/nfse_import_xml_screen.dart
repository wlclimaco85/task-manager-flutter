...
import '../../services/nfse_xml_import_caller.dart';
import '../../utils/app_logger.dart';
import '../../utils/grid_colors.dart';
import '../../widgets/nfse_xml_preview_widget.dart';

/// Importacao de XML de NFS-e (nota de servico), analoga a
/// WebNfeImportXmlScreen (importacao de XML de NF-e) -- mesmo fluxo
/// selecionar -> preview -> confirmar.
class WebNfseImportXmlScreen extends StatefulWidget {
  const WebNfseImportXmlScreen({super.key});

  @override
  State<WebNfseImportXmlScreen> createState() => _WebNfseImportXmlScreenState();
}

class _WebNfseImportXmlScreenState extends State<WebNfseImportXmlScreen> {
  ...
  Future<void> _selecionarXml() async {
    try {
      ...
      AppLogger.i.error('WebNfseImportXmlScreen: erro ao selecionar arquivo', st);
  ...
  Future<void> _carregarPreview() async {
    ...
    if (result.success) {
      ...
      AppLogger.i.warn('WebNfseImportXmlScreen: preview falhou -- ${result.message}');
      setState(() {
        _carregando = false;
        _sucesso = false;
  ...
  Future<void> _confirmarImportacao({int? produtoId, bool criarNovoProduto = false}) async {
    ...
    if (!result.success) {
      AppLogger.i.warn('WebNfseImportXmlScreen: confirmacao falhou -- ${result.message}');
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Importar XML NFS-e'),
        elevation: 0,
      ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 12),
            Text(
              'Selecione um arquivo XML de NFS-e (padrao ABRASF) para importar.\n'
              'Apos selecionar, clique em "Carregar e Visualizar" para pre-visualizar\n'
              'os dados da nota antes de confirmar a importacao.',
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Selecionar Arquivo XML',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
