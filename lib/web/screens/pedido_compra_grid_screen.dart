...
class WebPedidoCompraGridScreen extends StatefulWidget {
  const WebPedidoCompraGridScreen({super.key});

  @override
  State<WebPedidoCompraGridScreen> createState() => _WebPedidoCompraGridScreenState();
}

class _WebPedidoCompraGridScreenState extends State<WebPedidoCompraGridScreen> {
  ...
  final _statusOptions = [
    'Todos', 'RASCUNHO', 'EMITIDO', 'APROVADO', 'RECEBIDO_PARCIAL', 'RECEBIDO_TOTAL', 'CANCELADO'
  ];
  ...
  Future<void> _load() async {
    ...
    final data = await PedidoCompraService.fetchAll(
      ...
      dataInicio: _dataInicio != null ? _dataInicio!.toIso8601String().substring(0, 10) : null,
      dataFim: _dataFim != null ? _dataFim!.toIso8601String().substring(0, 10) : null,
    );
  ...
  void _openForm(Map<String, dynamic>? item) {
    showDialog(context: context, builder: (_) => PedidoCompraFormDialog(item: item, onSaved: _load));
  }

  void _showHistorico(List<PedidoCompraHistorico> historico) {
    showDialog(context: context, builder: (_) => PedidoCompraHistoricoDialog(historico: historico));
  }
  ...
  void _showReceberParcial(Map<String, dynamic> pedido) {
    ...
    if (id == null) return;
    showDialog(context: context, builder: (_) => ReceberDialog(pedidoId: id, itens: itens, onSaved: _load));
  }
        ...
        children: [
          ...
          SizedBox(width: 200, height: 36,
            child: TextField(
              ...
              decoration: const InputDecoration(hintText: 'Buscar fornecedor...', contentPadding: EdgeInsets.symmetric(horizontal: 8, vertical: 4), border: OutlineInputBorder(), isDense: true),
          ...
          SizedBox(height: 36,
            child: OutlinedButton.icon(
              onPressed: () { setState(() { _statusFilter = 'Todos'; _fornecedorFilter = ''; _fornecedorCtrl.clear(); _dataInicio = null; _dataFim = null; }); _load(); },
  ...
  Widget _buildTable() {
    ...
    if (_pedidos.isEmpty) return const Center(child: Text('Nenhum pedido de compra encontrado'));
  ...
  Widget _buildActions(int? id, String status, List<PedidoCompraHistorico> historico, Map<String, dynamic> pedido) {
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (status == 'RASCUNHO') ...[
        ...
        _actionIcon(Icons.send, 'Emitir', Colors.blue, () => _confirmAction('Emitir Pedido', 'Deseja emitir este pedido de compra?', () => PedidoCompraService.emitir(id))),
      ],
      if (status == 'EMITIDO') ...[
        _actionIcon(Icons.check_circle, 'Aprovar', Colors.green, () => _confirmAction('Aprovar Pedido', 'Deseja aprovar este pedido?', () => PedidoCompraService.aprovar(id))),
        _actionIcon(Icons.block, 'Cancelar', Colors.brown, () => _confirmAction('Cancelar Pedido', 'Deseja cancelar este pedido?', () => PedidoCompraService.cancelar(id))),
      ],
      if (status == 'APROVADO') ...[
        _actionIcon(Icons.inventory, 'Receber Parcial', Colors.orange, () => _showReceberParcial(pedido)),
        _actionIcon(Icons.done_all, 'Receber Total', Colors.teal, () => _confirmAction('Receber Total', 'Deseja receber totalmente este pedido?', () => PedidoCompraService.receberTotal(id))),
      ],
