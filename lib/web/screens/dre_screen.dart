...
class WebDreScreen extends StatefulWidget {
  const WebDreScreen({super.key});

  @override
  State<WebDreScreen> createState() => _WebDreScreenState();
}

class _WebDreScreenState extends State<WebDreScreen> {
  ...
  Widget _buildTabela() {
    final receitas = _listFromJson(_dreData!['receitas'] ?? _dreData!['receitas']);
    final custos = _listFromJson(_dreData!['custos']);
    final despesas = _listFromJson(_dreData!['despesas']);
    final totalReceitas =
        _toDouble(_dreData!['totalReceitas'] ?? _dreData!['totalReceitas']);
    ...
    final resultadoAnterior = _toDouble(_dreData!['resultadoLiquidoAnterior']);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _secaoHeader(),
            const Divider(thickness: 2),
            _secaoLinhas('RECEITAS', receitas, totalReceitas, isFirst: true),
            const Divider(),
  ...
  Widget _secaoLinhas(
    ...
    final totalAnterior = itens.fold<double>(
      0,
      (sum, item) => sum + _toDouble(item['valorAnterior']),
    );
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 3,
            child: Text(_currency.format(valor), textAlign: TextAlign.right),
          ),
          if (_compararAnterior)
            Expanded(
              flex: 3,
              child: Text(
                _currency.format(valorAnterior),
                textAlign: TextAlign.right,
              ),
            ),
          if (_compararAnterior)
      ...
      children: [
        ...
        if (_compararAnterior)
          Expanded(
            flex: 3,
            child: Text(
              _currency.format(resultadoAnterior),
              textAlign: TextAlign.right,
              style: TextStyle(
                fontWeight: FontWeight.bold,
