...
class WebObrigacaoFiscalGridScreen extends StatelessWidget {
  final SecurityCheck hasPermission;

  const WebObrigacaoFiscalGridScreen({super.key, required this.hasPermission});
  ...
  static Future<List<Map<String, dynamic>>> _loadPeriodicidade() async {
    try {
      final r = await NetworkCaller().getRequest('${ApiLinks.baseUrl}/api/enums/PeriodicidadeEnum');
      if (!r.isSuccess || r.body == null) return _periodicidadeFallback();
      final body = r.body!;
      List lista = body is List ? body : (body['data'] is List ? body['data'] : []);
      if (lista.isEmpty) return _periodicidadeFallback();
      return lista.whereType<Map>().map((e) => {
        'id': e['value']?.toString() ?? '',
        'nome': e['label']?.toString() ?? e['value']?.toString() ?? '',
      }).toList();
    } catch (_) { return _periodicidadeFallback(); }
  }
  ...
  Widget build(BuildContext context) {
    return DynamicGridWindowsScreen<ObrigacaoFiscal>(
      ...
      fieldOverrides: const [
        ...
        // Suprime o campo "regime_tributario" gerado pelo banco (nome diferente do override "regime")
        FieldConfigWindows(fieldName: 'regime_tributario',    label: '', isInForm: false, isVisibleByDefault: false, enabled: false),

        // ── Periodicidade → dropdown do banco (/api/enums/PeriodicidadeEnum) ──
        FieldConfigWindows(
          label: 'Periodicidade',
        ...
        // ── Checkboxes — forçar enabled=true ─────────────────────────────
        FieldConfigWindows(
          label: 'Ativo',
          fieldName: 'ativo',
