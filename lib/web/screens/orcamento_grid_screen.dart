...
class WebOrcamentoGridScreen extends StatefulWidget {
  const WebOrcamentoGridScreen({super.key});

  @override
  State<WebOrcamentoGridScreen> createState() => _WebOrcamentoGridScreenState();
}

class _WebOrcamentoGridScreenState extends State<WebOrcamentoGridScreen> {
  ...
  final _statusOptions = [
    'Todos', 'RASCUNHO', 'APROVADO', 'REPROVADO', 'CONVERTIDO', 'CANCELADO'
  ];
  ...
  void _openForm(Map<String, dynamic>? item) {
    showDialog(context: context, builder: (_) => OrcamentoFormDialog(item: item, onSaved: _load));
  }

  void _showHistorico(List<OrcamentoHistorico> historico) {
    showDialog(context: context, builder: (_) => OrcamentoHistoricoDialog(historico: historico));
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(
                hintText: 'Buscar cliente...', contentPadding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () {
                setState(() { _statusFilter = 'Todos'; _clienteFilter = ''; _clienteCtrl.clear(); _dataInicio = null; _dataFim = null; });
                _load();
              },
  ...
  Widget _buildTable() {
    ...
    if (_orcamentos.isEmpty) return const Center(child: Text('Nenhum orçamento encontrado'));
  ...
  Widget _buildActions(int? id, String status, List<OrcamentoHistorico> historico) {
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (status == 'RASCUNHO') ...[
        ...
        _actionIcon(Icons.check_circle, 'Aprovar', Colors.green, () => _confirmAction('Aprovar Orçamento', 'Deseja aprovar este orçamento?', () => OrcamentoService.aprovar(id))),
        _actionIcon(Icons.cancel, 'Reprovar', Colors.red, () => _confirmAction('Reprovar Orçamento', 'Deseja reprovar este orçamento?', () => OrcamentoService.reprovar(id))),
      ],
      if (status == 'APROVADO')
        _actionIcon(Icons.transform, 'Converter', Colors.blue, () => _confirmAction('Converter Orçamento', 'Deseja converter este orçamento em pedido?', () => OrcamentoService.converter(id))),
      _actionIcon(Icons.block, 'Cancelar', Colors.orange, () => _confirmAction('Cancelar Orçamento', 'Deseja cancelar este orçamento?', () => OrcamentoService.cancelar(id))),
