...
String _normalizeTelaNome(String value) =>
...
class SecurityMatrix {
  ...
  /// true quando o login trouxe a lista RBAC do backend.
  /// Lista vazia é snapshot válido e significa sem acesso configurado.
  final bool _hasBackendSnapshot;
  ...
  const SecurityMatrix._({
    ...
    bool hasBackendSnapshot = false,
    Map<String, bool> moduloAcaoCache = const {},
  })  : _backendPerms = backendPerms,
        _hasBackendSnapshot = hasBackendSnapshot,
        _moduloAcaoCache = moduloAcaoCache;
  ...
  factory SecurityMatrix.of(LoginModel? userInfo) {
    ...
    if (tipoLogin == LoginEnum.MASTER) {
      return SecurityMatrix._(
        ...
        backendPerms: {},
        hasBackendSnapshot: userInfo.permissoes != null,
      );
    }
    ...
    if (userInfo.permissoes != null && userInfo.permissoes!.isNotEmpty) {
      for (final p in userInfo.permissoes!) {
        final telaNome = _normalizeTelaNome(p.telaNome);
        final existing = backendPerms[telaNome] ?? <AppAction>{};
        ...
        backendPerms[telaNome] = existing;
      }
    }
  ...
  bool _can(AppScreen screen, AppAction action) {
    ...
    // Se backend retornou snapshot de permissões, usa ele como fonte de verdade.
    // Snapshot vazio significa role sem telas liberadas.
    if (_hasBackendSnapshot) {
      final perms = _backendPerms[_normalizeTelaNome(screen.name)];
      if (perms == null) return false;
      if (!perms.contains(action)) return false;
      return !ModuloAccess.hasModulosConfigurados ||
  ...
  Set<String> get viewableTelaIds {
    ...
    _backendPerms.forEach((tela, actions) {
      ...
      if (ModuloAccess.hasModulosConfigurados) {
        final screen = AppScreen.values
            .where((s) => _normalizeTelaNome(s.name) == tela)
            .firstOrNull;
  ...
  Set<String>? allowedTelaIds(Set<String> allKnownIds) {
    ...
    if (!_hasBackendSnapshot) {
      // Se nao ha snapshot do backend, usa fallback da matrix hardcoded.
    ...
    if (_backendPerms.isEmpty) return <String>{};
    final normalizedViewable = viewableTelaIds.map(_normalizeTelaNome).toSet();
    return allKnownIds
        .where((id) => normalizedViewable.contains(_normalizeTelaNome(id)))
        .toSet();
  }
  ...
  bool hasAnyAccess(AppScreen screen) {
    if (profile == UserProfile.system || tipoLogin == LoginEnum.MASTER)
      return true;
    if (_hasBackendSnapshot) {
      return (_backendPerms[_normalizeTelaNome(screen.name)]?.isNotEmpty) ??
          false;
    }
...
class ModuloAccess {
  ...
  static Future<void> load() async {
    ...
    if (parceiroId != null) {
      try {
        ...
        if (resp.statusCode == 200) {
          final List<dynamic> data = jsonDecode(resp.body);
          parceiroModulos =
              data.map((m) => m['nome']?.toString() ?? '').toSet();
        } else {
    ...
    if (_modulosContratados.isNotEmpty &&
        !_modulosContratados.contains('Financeiro')) {
  ...
  static bool get hasModulosConfigurados =>
      _loaded && _modulosContratados.isNotEmpty;
