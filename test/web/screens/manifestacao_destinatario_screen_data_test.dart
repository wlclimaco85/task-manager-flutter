...
/// Testes de widget da tela de Manifestação do Destinatário (web) COM dados
/// reais mockados via `http.runWithClient` (mesma técnica já usada em
/// test/services/manifestacao_caller_test.dart). Os testes existentes em
/// manifestacao_destinatario_screen_test.dart só exercitam o caminho de erro
/// (sem rede); estes cobrem o caminho de sucesso: badges dos 4 tipos e 3
/// status, DataTable/cards populados, estado vazio, tooltip de erro no
/// histórico, troca de aba, e o fluxo completo de submissão (sucesso e
/// falha) — os gaps reais apontados pelo QA na 3ª reprovação (card P2-502).
void main() {
  ...
  group('ManifestacaoDestinatarioScreen (web) - pendências com dados reais',
      () {
    testWidgets(
      ...
      // Status default (sem campo status explícito != PENDENTE) usa o badge
      // "Pendente" com ícone de relógio — 4 ocorrências, uma por linha.
    ...
    testWidgets('layout mobile (360px) mostra cards com chave e justificativa',
        (tester) async {
      final mockClient = MockClient((request) async {
        if (request.url.toString().contains('pendentes')) {
          return http.Response(
      ...
      expect(find.text('Motivo de teste'), findsOneWidget);
    });
  });

  group('ManifestacaoDestinatarioScreen (web) - histórico com dados reais',
      () {
    testWidgets(
  ...
  group(
      'ManifestacaoDestinatarioScreen (web) - fluxo completo de submissão (POST real mockado)',
      () {
    testWidgets(
      ...
      await http.runWithClient(() async {
        await tester.pumpWidget(wrap(const ManifestacaoDestinatarioScreen()));
        await tester.pumpAndSettle(const Duration(seconds: 2));
        // Consome as chamadas iniciais de initState antes de contar as do fluxo.
        getPendentesCount = 0;
        getHistoricoCount = 0;
        ...
        await tester
            .tap(find.widgetWithText(ElevatedButton, 'Confirmar'));
        // pump() em vez de pumpAndSettle: o SnackBar tem timer de
        // auto-dismiss e pumpAndSettle(duration) avança frames até o timer
        // esgotar, fazendo o SnackBar desaparecer antes da asserção. Pumps
        // sucessivos drenam cada await da cadeia (POST -> snackbar -> GET
        // pendentes -> GET histórico) sem deixar o timer do SnackBar vencer.
        for (var i = 0; i < 6; i++) {
    ...
    testWidgets(
      ...
      // Sem reload das listas em caso de erro (só recarrega em sucesso).
      expect(getCount, 0);
    });
  });
}
