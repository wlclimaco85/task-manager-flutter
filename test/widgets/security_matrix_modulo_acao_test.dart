...
void main() {
  ...
  // ── GED (modulo gratuito) ────────────────────────────────────────────────
  // Bug real reportado pelo usuario (video): GED e' modulo GRATUITO -- nao
  // precisa ser contratado pela empresa -- mas ficava mapeado em
  // _moduloToScreens, entao isScreenAllowed() bloqueava a tela quando a
  // empresa nao tinha 'GED' na lista de modulos contratados, MESMO com a
  // permissao de role (podeVer) 100% liberada. Efeito: aba GED em branco,
  // sem nenhum erro visivel.
  group('GED — modulo gratuito, nunca bloqueado por modulo contratado', () {
    test('cliente com role OK mas SEM GED nos modulos contratados ainda ve GED',
        () {
      final info = buildCliente(
        // Empresa so' contratou Financeiro -- GED nao esta na lista.
        modulosContratados: ['Financeiro'],
        permissoes: [
          RolePermissaoItem(
              telaNome: 'ged',
    ...
    test('cliente SEM permissao de role pra GED continua sem ver (RBAC normal)',
        () {
      final info = buildCliente(
        modulosContratados: ['Financeiro'],
        permissoes: [
      ...
      final matrix = SecurityMatrix.of(info);
      expect(matrix.canView(AppScreen.ged), isFalse);
    });
  });
}
