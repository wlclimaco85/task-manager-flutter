...
void main() {
  testWidgets('NFSe detail usa secoes sequenciais no web/desktop', (tester) async {
  ...
  testWidgets('NFSe detail alterna cabecalho para formulario de servicos', (tester) async {
    await tester.pumpWidget(_wrap(const NfseDetailScreen(item: {})));
    await tester.pump();

    final novoButton = find.widgetWithText(ElevatedButton, 'Novo').first;
    await tester.scrollUntilVisible(novoButton, 300, scrollable: find.byType(Scrollable).first);
    await tester.pump();
  ...
  testWidgets('NFSe detail abre busca inline sem navegar para modal de tela cheia', (tester) async {
    await tester.pumpWidget(_wrap(const NfseDetailScreen(item: {})));
    await tester.pump();
